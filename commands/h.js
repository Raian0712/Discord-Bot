...
const channels = {};
const category = ['Country', 'Capital City', 'Food', 'Movie', 'Band', 'Animal', 'IT', 'Compound Word', 'Pokemon', 'Debug'];
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const hangmanMenu = new Discord.MessageEmbed()
            ...
            .setTimestamp()
            .setDescription(`
                1. Country          (${words.country.length} words)
                2. Capital City     (${words.capitalcity.length} words)
                3. Food             (${words.food.length} words)
                4. Movie            (${words.movie.length} words)
                5. Band             (${words.band.length} words)
                6. Animal           (${words.animal.length} words)
                7. IT               (IT words) (${words.computer.length} words)
                8. Compound Words   (2 words combined into 1, ex. catfish) (9${words.compoundWord.length} words)
                9. Pokemon          (${words.pokemon.length} words)
                10. Debug           (al'something&) (${words.debug.length} words)

                Choose your options with \`${prefix}h [1-9].\` 
            `);

        if (args.length == 1 && !channels[message.channel.id].gameOn) {
            if (!Number.isNaN(args[0]) && Number.isInteger(parseInt(args[0])) && args[0] >= 1 && args[0] <= 10) {
                ...
                switch (channels[message.channel.id].input) {
                    ...
                    case 10: {
                        channels[message.channel.id].playWord = selectWord(words.debug);
                        break;
                    }
                }
                ...
                for (const i in channels[message.channel.id].playWord) {
                    if (channels[message.channel.id].playWord[i] != ' ' && channels[message.channel.id].playWord[i] != `'` && channels[message.channel.id].playWord[i] != '&'
                    && channels[message.channel.id].playWord[i] != ',') {
                        channels[message.channel.id].wordlength++;
                        channels[message.channel.id].displayWord.push('_');
                    } else if (channels[message.channel.id].playWord[i] == `'`) {
        ...
        if (channels[message.channel.id].gameOn) {
            if (args[0] == 'guess') {
                ...
                if (args[1].length == 1 && args[1] != ' ' && args[1] != '\n') {
                    console.log(channels[message.channel.id].guessChar);
                    console.log(args[1]);
                    console.log(channels[message.channel.id].guessChar.indexOf(args[1]));
                    if (channels[message.channel.id].playWord.includes(args[1]) && channels[message.channel.id].guessChar.indexOf(args[1]) == -1) {
                    ...
                    } else if (channels[message.channel.id].guessChar.indexOf(args[1]) != -1) {
                        message.channel.send("Repeating character guess!");
                    } else {
